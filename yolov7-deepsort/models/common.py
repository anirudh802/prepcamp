        ...
        if isinstance(branch, nn.Sequential):
            ...
            if not hasattr(self, "id_tensor"):
                ...
                kernel_value = np.zeros(
                    (self.in_channels, input_dim, 3, 3), dtype=float
                )
...
class RepConv_OREPA(nn.Module):
    ...
    def _fuse_bn_tensor(self, branch):
        ...
        if not isinstance(branch, nn.BatchNorm2d):
            ...
            if not hasattr(self, 'id_tensor'):
                input_dim = self.in_channels // self.groups
                kernel_value = np.zeros((self.in_channels, input_dim, 3, 3), dtype=float)
                for i in range(self.in_channels):
                    kernel_value[i, i % input_dim, 1, 1] = 1
                self.id_tensor = torch.from_numpy(kernel_value).to(branch.weight.device)
            kernel = self.id_tensor
...
class WindowAttention_v2(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.,
        ...
        relative_coords_h = torch.arange(-(self.window_size[0] - 1), self.window_size[0], dtype=torch.float)
        relative_coords_w = torch.arange(-(self.window_size[1] - 1), self.window_size[1], dtype=torch.float)
