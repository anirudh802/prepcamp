...
def postprocess(num_dets, det_boxes, det_scores, det_classes, img_w, img_h, input_shape, letter_box=True):
    boxes = det_boxes[0, :num_dets[0][0]] / np.array([input_shape[0], input_shape[1], input_shape[0], input_shape[1]], dtype=float)
    ...
    boxes = boxes * np.array([old_w, old_h, old_w, old_h], dtype=float)
    if letter_box:
        boxes -= np.array([offset_w, offset_h, offset_w, offset_h], dtype=float)
